...
<figure>
	<a href={idk.src}>
		<Image src={idk} formats={['avif', 'webp']} alt="A description of my image."
					 widths={[400, 800]}
					 width={800}
					 sizes={`(max-width: 720px) 400px, (max-width: 1600px) 800px`}
		/>
	</a>
	<figcaption>Final product sitting on my son's nightstand</figcaption>
</figure>

# Usage
<figure>
	<SimpleLineGraph
		sql="select created_at::date date, count(*) count from events where event_name = 'toggle_switched_v1' group by created_at::date order by created_at::date" />
	<figcaption>How many times switched</figcaption>
</figure>
...
<p class="terminal-alert terminal-alert-primary">Had to remove switch covers as my 3-year-old had a hard time flipping
	them.</p>
