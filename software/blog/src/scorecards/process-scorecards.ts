...
import { HOLE_PROPS, type RawUdiscScorecardEntry } from './raw-udisc-scorecard-entry'
import type { ParseScorecardInput, ScorecardResponse } from './scorecard'
import { getPlayerTimeInYears } from './get-player-time-in-years'

export async function parseScorecardFromCsv(csv: string): Promise<RawUdiscScorecardEntry[]> {
...
export async function processScorecards({
	...
	const howLongHaveYouBeenPlaying = earliestDate?.date
		? 'None'
		: getPlayerTimeInYears(new Date(earliestDate?.date!))
