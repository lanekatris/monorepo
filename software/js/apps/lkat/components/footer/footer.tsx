...
export function Footer() {
  return (
    <footer className="flex flex-col items-center justify-between px-6 py-4 bg-white dark:bg-gray-800 sm:flex-row sticky top-[100vh]">
      <Link href="/">
        <a className="text-xl  text-gray-500 dark:text-white hover:text-gray-700 dark:hover:text-gray-300">
          Lane's Website
        </a>
      </Link>

      <p className="py-2 text-gray-500 dark:text-white sm:py-0">
      ...
      <div className="flex -mx-2">
        {footerData.map(({ href, label, type }) => (
          <a
            key={type}
            href={href}
            className="mx-2 dark:text-gray-300 hover:text-gray-500 dark:hover:text-gray-300"
            aria-label={label}
          >
            {footerSvgMap[type]}
          </a>
